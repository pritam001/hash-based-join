...
# hashes using y = (k + l least significant bits of the key) mod(k)
# number of buckets = k
# round_no = l
# hash value is increased by 1 due to prev implementation of bucket numbers starting from 1, 2, 3, ...
def hash_func2(l, k, key):
    temp = bin(key).split("0b")[1]
    for i in range(0, 16 - len(bin(key).split("0b")[1]), 1):
        temp = '0' + temp
    temp2 = 0
    lsb = temp[16 - k - l:]
    for j in range(0, len(lsb), 1):
        # print str(lsb[j]) + " | " + str(j)
        temp2 += int(lsb[j]) * pow(2, len(lsb) - j - 1)
        # print str(temp2) + " += " + str(int(lsb[j])) + " * " + str(pow(2, len(lsb) - j - 1)) + " | " + str(j)
    temp3 = temp2 % k
    print str(key) + " | " + str(temp) + " | " + str(lsb) + " | " + str(temp2) + "%" + str(k) + " = " + str(temp3)
    return temp3 + 1
...
def hashing_round(round_no, total_buckets, int_list1, int_list2):
    ...
    for i in range(0, len(int_list1), 1):
        x = hash_func2(round_no, total_buckets, int_list1[i])
    ...
    for j in range(0, len(int_list2), 1):
        x = hash_func2(round_no, total_buckets, int_list2[j])
    ...
    for i in range(0, total_buckets, 1):
        if page_count_list1[i] == 0:
            print "Round " + str(round_no) + " Bucket " + str(i + 1) + ": Bucket from R1 is empty. No further processing required.\n"
            continue
        if page_count_list2[i] == 0:
            print "Round " + str(round_no) + " Bucket " + str(i + 1) + ": Bucket from R2 is empty. No further processing required.\n"
            continue
        if page_count_list1[i] + page_count_list2[i] <= no_of_available_pages - 1:
            print "Round " + str(round_no) + " Bucket " + str(i + 1)
            matching_pair_list = []
            ...
            for j in range(0, len(list1), 1):
                for k in range(0, len(list2), 1):
                    if list1[j] == list2[k]:
                        matching_pair_list.append((list1[j], list2[k]))

            print "R1 keys : " + str(list1) + " Pages : " + str(page_count_list1[i])
            print "R2 keys : " + str(list2) + " Pages : " + str(page_count_list2[i])
            print "In memory join : Yes"
            # print "Total size is " + str(page_count_list1[i] + page_count_list2[i]) + " pages"
